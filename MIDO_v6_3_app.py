...
def init_db():
    ...
    ensure_columns(conn, "chat_history", {"role":"TEXT", "message":"TEXT", "created_at":"TEXT", "conversation_id":"TEXT", "source":"TEXT DEFAULT 'text'"})
    ensure_columns(conn, "chat_sessions", {"title":"TEXT", "created_at":"TEXT", "updated_at":"TEXT"})
...
def _new_conversation(title="محادثة جديدة"):
    cid = uuid.uuid4().hex
    now = now_text()
    execute("INSERT INTO chat_sessions (id,title,created_at,updated_at) VALUES (?,?,?,?)", (cid,title,now,now))
    st.session_state["mido_active_conversation"] = cid
    return cid


def get_active_conversation():
    cid = st.session_state.get("mido_active_conversation")
    if cid:
        exists = fetch_df("SELECT id FROM chat_sessions WHERE id=? LIMIT 1", (cid,))
        if not exists.empty:
            return cid
    recent = fetch_df("SELECT id FROM chat_sessions ORDER BY updated_at DESC, created_at DESC LIMIT 1")
    if not recent.empty:
        cid = str(recent.iloc[0]["id"])
        st.session_state["mido_active_conversation"] = cid
        return cid
    return _new_conversation()


def save_chat_message(conversation_id, role, message, source="text"):
    message = str(message or "").strip()
    if not message:
        return
    execute("INSERT INTO chat_history (role,message,created_at,conversation_id,source) VALUES (?,?,?,?,?)",
            (role,message,now_text(),conversation_id,source))
    execute("UPDATE chat_sessions SET updated_at=? WHERE id=?", (now_text(),conversation_id))
    # Give a new conversation a useful title from the first user message.
    if role == "user":
        row = fetch_df("SELECT title FROM chat_sessions WHERE id=?", (conversation_id,))
        if not row.empty and (not str(row.iloc[0]["title"] or "").strip() or str(row.iloc[0]["title"]).startswith("محادثة جديدة")):
            title = re.sub(r"\s+", " ", message).strip()[:55]
            execute("UPDATE chat_sessions SET title=?,updated_at=? WHERE id=?", (title,now_text(),conversation_id))


def conversation_messages(conversation_id, limit=40):
    df = fetch_df("SELECT role,message,created_at,source FROM chat_history WHERE conversation_id=? ORDER BY id DESC LIMIT ?", (conversation_id,limit))
    if df.empty:
        return []
    return df.iloc[::-1].to_dict("records")


def ask_real_mido(question, conversation_id=None):
    client, model = get_ai_client()
    if not client:
        raise RuntimeError("AI غير مربوط بعد.")
    ctx = database_context_for_ai()
    prompt = (
        "You are MIDO, the user's private business assistant. Maintain a natural multi-turn conversation and remember "
        "the recent conversation supplied below. Answer in Iraqi Arabic unless the user asks for another language. "
        "Be concise but helpful. For business facts, use ONLY the supplied ERP data and the user's messages; never invent values. "
        "If the data is insufficient, say what is missing. Received/delivered shipments should not be described as active. "
        "When the user refers to something from an earlier turn (for example 'هاي الشركة' or 'والدفعة الثانية'), resolve it from the conversation context.\n"
        "ERP DATA:\n" + json.dumps(ctx, ensure_ascii=False, default=str)
    )
    messages = [{"role":"system","content":prompt}]
    if conversation_id:
        for item in conversation_messages(conversation_id, limit=36):
            role = item.get("role")
            if role in ("user","assistant") and item.get("message"):
                messages.append({"role":role,"content":str(item["message"])})
    messages.append({"role":"user","content":str(question)})
    r = client.chat.completions.create(model=model, messages=messages, temperature=0.15)
...
def render_browser_speech(text, key="mido_speech", autoplay=False):
    ...
    <script>
      ...
      if ({"true" if autoplay else "false"}) {{
        setTimeout(() => document.getElementById("speak").click(), 250);
      }}
    </script>
    """
...
st.sidebar.caption("مساعد محمد التجاري — v6.3 Chat-Style AI")
st.sidebar.markdown("---")
menu = [
    "📊 لوحة التحكم",
    "🏭 الشركات الصينية",
...
# -------------------- MIDO AI --------------------
    ...
    st.info("✨ V6.3 — واجهة محادثة موحّدة: كتابة + صوت + ملفات + ذاكرة محادثة مستمرة")
    tab_chat, tab_files, tab_dev, tab_history = st.tabs(["💬 محادثة ميدو", "📎 الملفات الذكية", "🛠️ مطور ميدو", "🕘 سجل AI"])

    active_cid = get_active_conversation()

    with tab_chat:
        st.markdown("### 💬 ميدو — محادثة مستمرة")
        st.caption("مثل تطبيقات الدردشة: اكتب أو سجّل صوتك، وميدو يتذكر نفس المحادثة ويكمل وياك سؤال بعد سؤال.")

        sessions = fetch_df("SELECT id,title,updated_at FROM chat_sessions ORDER BY updated_at DESC, created_at DESC LIMIT 30")
        csel1, csel2 = st.columns([4,1])
        with csel1:
            if not sessions.empty:
                labels = {f"{(r.title or 'محادثة')[:42]} — {r.updated_at or ''}": str(r.id) for r in sessions.itertuples()}
                current_label = next((k for k,v in labels.items() if v == active_cid), list(labels.keys())[0])
                picked = st.selectbox("المحادثات", list(labels.keys()), index=list(labels.keys()).index(current_label), key="mido_session_picker")
                if labels[picked] != active_cid:
                    st.session_state["mido_active_conversation"] = labels[picked]
                    st.rerun()
        with csel2:
            if st.button("＋ جديدة", key="new_unified_chat", use_container_width=True):
                _new_conversation()
                st.rerun()

        st.markdown("---")
        msgs = conversation_messages(active_cid, limit=100)
        if not msgs:
            st.info("ابدأ برسالة نصية أو صوتية. ميدو راح يتذكر سياق المحادثة ويكمل وياك.")
        for idx, m in enumerate(msgs):
            role = "assistant" if m.get("role") == "assistant" else "user"
            with st.chat_message(role):
                if role == "user" and m.get("source") == "voice":
                    st.caption("🎙️ رسالة صوتية")
                st.markdown(m.get("message") or "")
                if role == "assistant":
                    render_browser_speech(m.get("message") or "", key=f"chat_speak_{idx}")

        st.markdown("#### 🎙️ أرسل صوت")
        audio = st.audio_input("اضغط المايك وسجّل رسالتك", key=f"mido_unified_audio_{active_cid}")
        if audio is not None and ai_ready() and voice_ready():
            raw = audio.getvalue()
            digest = hashlib.sha256(raw).hexdigest()
            digest_key = f"_unified_voice_digest_{active_cid}"
            if st.session_state.get(digest_key) != digest:
                st.session_state[digest_key] = digest
                try:
                    with st.spinner("ميدو يسمعك ويفكر..."):
                        voice_text = transcribe_audio_bytes(raw, getattr(audio, "name", "voice.wav"))
                        save_chat_message(active_cid, "user", voice_text, "voice")
                        ans = ask_real_mido(voice_text, active_cid)
                        save_chat_message(active_cid, "assistant", ans, "voice")
                    st.session_state["_autoplay_latest_voice"] = ans
                    st.rerun()
                except Exception as e:
                    st.error(f"تعذر تشغيل الصوت: {e}")

        latest_voice = st.session_state.pop("_autoplay_latest_voice", None)
        if latest_voice:
            st.success("ميدو جاوبك — وتكدر تكمل برسالة ثانية مباشرة.")
            render_browser_speech(latest_voice, key="autoplay_latest_voice", autoplay=True)

        st.markdown("#### ⌨️ أو اكتب")
        chat_uploads = st.file_uploader("📎 أرفق ملفات ضمن نفس المحادثة (اختياري)", type=["pdf","png","jpg","jpeg","webp","xlsx","xls","csv","txt"], accept_multiple_files=True, key=f"unified_chat_files_{active_cid}")
        q = st.chat_input("اكتب رسالتك لميدو...")
        if q is not None and ai_ready():
            q = q.strip()
            if q or chat_uploads:
                try:
                    if chat_uploads:
                        ...
                        ans = analysis.get("summary") or "تم تحليل الملفات. تقدر تراجع التفاصيل في تبويب الملفات الذكية."
                    else:
                        with st.spinner("ميدو يفكر..."):
                            ans = ask_real_mido(q, active_cid)
                    save_chat_message(active_cid, "user", q or "[ملفات مرفوعة]", "text")
                    save_chat_message(active_cid, "assistant", ans, "text")
                    st.rerun()
                except Exception as e:
                    st.error(f"تعذر تشغيل MIDO AI: {e}")

        st.caption("ملاحظة: بسبب حماية المايك في المتصفح، كل رسالة صوتية تحتاج ضغطة تسجيل جديدة. لكن الجواب يصير تلقائياً وتبقى المحادثة نفسها مستمرة.")
    ...
    # One persistent conversation can continue across text and voice tabs.
    with tab_dev:
