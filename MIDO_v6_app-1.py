...
def voice_ready():
    return bool(_ai_secret("transcription_model"))


def tts_ready():
    return bool(_ai_secret("tts_model"))
...
def synthesize_speech(text):
    client, _model = get_ai_client()
    model = _ai_secret("tts_model")
    if not client or not model:
        raise RuntimeError("فعّل tts_model داخل [ai] في Streamlit Secrets.")
    voice = _ai_secret("tts_voice", "alloy")
...
st.sidebar.caption("مساعد محمد التجاري — v6 AI Suite")
st.sidebar.markdown("---")
menu = [
    "📊 لوحة التحكم",
    "🏭 الشركات الصينية",
...
# -------------------- MIDO AI --------------------
    ...
    with tab_chat:
        ...
        if st.session_state.get("last_mido_answer"):
            ...
            if tts_ready() and st.button("🔊 اسمع الجواب", key="speak_chat"):
                try:
                    st.audio(synthesize_speech(st.session_state["last_mido_answer"]), format="audio/mp3")
                except Exception as e:
    ...
    with tab_voice:
        ...
        if not voice_ready():
            st.info("حتى يتحول الصوت إلى نص، أضف transcription_model داخل [ai] في Streamlit Secrets. وللرد الصوتي أضف tts_model و tts_voice.")
        ...
        if voice_text:
            ...
            if st.session_state.get("_last_voice_answer"):
                ...
                if tts_ready():
                    try:
                        st.audio(synthesize_speech(st.session_state["_last_voice_answer"]), format="audio/mp3", autoplay=False)
                    except Exception as e:
                        st.warning(f"الرد النصي شغال، لكن الرد الصوتي تعذر: {e}")
